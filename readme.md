...
созданной / существующего файла удерживайте Shift при щелчке правой кнопке мыши на файле / ярлыке на `.csmdb23` файл, после чего
...
если параметр `markinv` - приложение "Учёт клейм", на а `csmmain` соответствует реакции по умолчанию запуск приложения "АИС
Метроконтроль".
